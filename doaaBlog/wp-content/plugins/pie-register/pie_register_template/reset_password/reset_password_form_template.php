	...
	class Reset_pass_form_template
	{
		...
		function add_new_confirm_pass(){
			$data  = '<p class="field password_field">';
				$data .= '<label for="pass1">'.esc_html__("New password","pie-register").'</label>';
			...
			$data .= '</p>';
			$data .= '<p class="field">';
		  	$data .= '<label for="pass2">'.esc_html__("Confirm new password","pie-register").'</label>';
		...
		function add_submit(){
			...
			$data .= '<input type="submit" name="wp-submit" id="wp-reset-submit" class="button button-primary button-large" value="'.esc_attr__("Reset Password","pie-register").'">';
			$data .= '</div>';
			return $data;
		}
		function add_login_register($pagenow){
			$data  = '<div class="field">';
				$data .= '<p class="nav">';
					$data .= '<a href="'.esc_url(pie_login_url()).'">'.esc_html__("Log in","pie-register").'</a>';
					$data .= ' | ';
					$data .= '<a href="'.esc_url(pie_registration_url()).'">'.esc_html__("Register","pie-register").'</a>';
				$data .= '</p>';
			$data .= '</div>';
			if(isset($pagenow) && $pagenow == 'wp-login.php' ){
				$data .= '<div class="backtoblog">';
					$data .= '<a title="'.esc_attr__("Are you lost?","pie-register").'" href="'.esc_url(get_bloginfo("url")).'">&larr; '.esc_html__("Back to","pie-register").' '.esc_html(get_bloginfo("name")).'</a>';
				$data .= '</div>';
			}
			return $data;
		}
		function add_email_or_username(){
			...
			{
				$data .= '<label for="user_login">'.((isset($this->pr_option['forgot_pass_username_label']) && !empty($this->pr_option['forgot_pass_username_label']))? esc_html($this->pr_option['forgot_pass_username_label']): esc_html__("Username or E-mail:","pie-register")).'</label>';
			}
		...
		function add_register_or_login($pagenow){
			...
			if(isset($pagenow) && $pagenow == 'wp-login.php'){
				...
				$forgot_pass_form .= '<a href="'.esc_url(wp_login_url()).'">'.esc_html__('Log in',"pie-register").'</a>';
				$forgot_pass_form .= ' | ';
				$forgot_pass_form .= '<a href="'.esc_url(wp_registration_url()).'">'.esc_html__('Register',"pie-register").'</a>';
				$forgot_pass_form .= '</p>';
				$forgot_pass_form .= '<p class="forgot_pass_links">';
				$forgot_pass_form .= '<a title="'.esc_attr__('Are you lost?',"pie-register").'" href="'.esc_url(get_bloginfo("url")).'">&larr; '.esc_html__('Back to',"pie-register").' '.esc_html(get_bloginfo("name")).'</a>';
				$forgot_pass_form .= '</p>';
			}
