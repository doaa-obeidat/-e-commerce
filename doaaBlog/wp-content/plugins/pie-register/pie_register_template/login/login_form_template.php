	...
	class Login_form_template
	{
		...
		function add_rememberme(){
			$form_data = '<p class="forgetmenot">';
				...
				$form_data .= '<label for="rememberme">';
					$form_data .= esc_html__("Remember Me","pie-register");
				$form_data .= '</label>';
			$form_data .= '</p>';
			return $form_data;
		}
		function add_submit(){
			$form_data = '<p class="submit">';
				$form_data .= '<input type="submit" value="'.esc_attr__("Log In","pie-register").'" class="button button-primary button-large" id="wp-login-submit" name="wp-submit">';
				$form_data .= '<input type="hidden" value="'.esc_url(admin_url()).'" name="redirect_to">';
		...
		function add_register_lostpassword($pagenow){
			$form_data = '<p id="nav">';
				...
				if($classPieRegister->is_anyone_can_register()){
					$form_data .= '<a href="'.esc_url(pie_registration_url()).'">'.esc_html__("Register","pie-register").'</a>';
					$form_data .= '<a style="cursor:default;text-decoration:none;" href="javascript:;"> | </a>';
				}
				$form_data .= '<a title="'.esc_attr__("Reset your password","pie-register").'" href="'.esc_url(pie_lostpassword_url()).'">'.esc_html__("Lost your password?","pie-register").'</a>';
			$form_data .= '</p>';
