...
<p><strong><?php esc_html_e("Note",'pie-register') ?>:</strong> <?php esc_html_e("Page settings on the Role Based Redirect tab will always override page settings on the All Users tab",'pie-register') ?>.</p>
<div class="settings piereg_added_area roles_container" style="padding-bottom:0px;margin-left:0px;">
<fieldset class="piereg_fieldset_area" <?php disabled($_disable, true); ?>>
  
      <legend><?php echo esc_html_e("Add Record",'pie-register') . wp_kses_post($_available_in_pro); ?></legend>
  ...
  <form method="post" id="redirect_form">
    ...
    <div class="fields" style="width:100%;">
      <div class="fields">
        <label for="piereg_user_role"><?php esc_html_e("User Role",'pie-register') ?></label>
        <?php
        ...
        <?php
			...
			foreach($role as $key=>$value) {
				...
				selected($piereg_user_role == $key, true);
    ...
    <div class="fields" style="width:100%;">
      <div class="fields">
        <label for="log_in_page">
          <?php esc_html_e("After Log In Page",'pie-register') ?>
        </label>
        <?php 
    ...
    <div class="fields" style="width:100%;">
      <div class="fields">
        <label for="log_out_page">
          <?php esc_html_e("After Log out Page",'pie-register') ?>
        </label>
        <?php 
    ...
    <div class="fields">
      <?php if(!$is_add_new){ ?>
          <input type="submit" class="submit_btn submit_btn_mar_ryt2" name="redirect_settings_update" value=" <?php esc_attr_e("Update","pie-register");?> " />
      ...
      <?php }else{?>
      	<input type="submit" class="submit_btn submit_btn_mar_ryt2" name="redirect_settings_add_new" value=" <?php esc_attr_e("Save Record","pie-register");?> " />
      <?php } ?>
    </div>
  </form>
</fieldset>
