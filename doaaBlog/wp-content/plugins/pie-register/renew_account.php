...
global $wp_session;
    ...
    $pr_data .= '<h2 class="pr_renew_headding">'.(esc_html__("Renew Account","pie-register")).'</h2>';
    $pr_data .= '<div style="text-align:right;"><span class="field_note">* '.(esc_html__("Required Field(s)","pie-register")).'</span></div>';
	$pr_data .= '</div>';
    $pr_data .= '<div id="piereg_login">';
	  ...
	  <input type="hidden" name="e_mail" value="'.esc_attr($user_array['email']).'" >
      	...
      	<ul id="pie_register">';
			if(!empty($user_array) && $is_renew)
			{
				$pr_data .= '<li class="fields">
					<div class="fieldset">
						<label>'.(esc_html__("User Name","pie-register")).'</label>
						<span>'.esc_html($user_array['username']).'</span>
					</div>
				</li>';
				$pr_data .= '<li class="fields">
					<div class="fieldset">
						<label>'.(esc_html__("E-mail","pie-register")).'</label>
						<span>'.esc_html($user_array['email']).'</span>
					</div>
				</li>';
			}
			else{
				$pr_data .= '<li class="fields">
					<div class="fieldset">
						<label for="x_card_num">'.(esc_html__("User Name","pie-register")).'</label>
						<input type="text" class="input_fields piereg_validate[required]" id="user_name" name="user_name" autocomplete="off" value="'.((isset($_POST['user_name']))?sanitize_user($_POST['user_name']):"").'" aria-required="true" aria-invalid="false">
			...
			$pr_data .= '<li class="fields">
				<div class="fieldset">
					<label for="x_card_num">'.(esc_html__("Select Payment","pie-register")).'</label>';
					...
					if(intval($piereg_form_pricing_fields['form_id_'.$user_registered_form_id]['field_as']) == 0){
						...
						$pr_data .= '<option value="">'.esc_html__("Select","pie-register").'</option>';
						$pr_data .= $allowed_payment_gateways;
