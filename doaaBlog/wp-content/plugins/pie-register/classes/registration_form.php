...
class Registration_form extends PieReg_Base
{
	...
	function validateRegistration($errors)
	{
		...
		*/
		...
		// Check if username field exist in form or not
		if(is_array($this->data))
		{
			$username_field_exist =  false;
			foreach($this->data as $field)
			{
			   if( $field['type'] == 'username' )
			   {
				   $username_field_exist = true;
				   break;
			   }
			}
		}
		...
		{
			$errors->add( "username" , '<strong>'.esc_html(ucwords(__('error','pie-register'))).'</strong>: '.apply_filters("piereg_Invalid_Username",__('Invalid Username','pie-register' )));
		}
		else if ( username_exists( $this->pie_post_array['username'] ) )
		{
			if ( $username_field_exist == true) 
			{
				$errors->add( "username" , '<strong>'.esc_html(ucwords(__('error','pie-register'))).'</strong>: '.apply_filters("piereg_Username_already_exists",__('Username already exists','pie-register' )));
			}
		}		
		...
		{
			$errors->add( "email" , '<strong>'.esc_html(ucwords(__('error','pie-register'))).'</strong>: '.apply_filters("piereg_Email_address_already_exists",__('E-mail address already exists','pie-register' )));
		}
		...
		if(is_array($this->data)){
			...
			*/
				if($field['type'] == "date" )
				{					
					$date = $this->pie_post_array[$field['type'] .'_'.$field['id']];	
								
					if( ($field['date_type'] == "datefield") || ($field['date_type'] == "datedropdown") )
					{
						if ( !empty($date['date']['mm']) && !empty($date['date']['dd']) && !empty($date['date']['yy']) )
						{
							$month     = $date['date']['mm']; 
							$date_val  = $date['date']['dd']; 
							$year      = $date['date']['yy']; 
							$validateDate = checkdate($month, $date_val, $year);

							if( $validateDate == false )
							{ 						
								$validation_message	= (!empty($field['validation_message']) ? $field['validation_message'] : __("Please enter a valid date.","pie-register"));
			...
			else if($field['type']=="invitation")
			{
				if($piereg["enable_invitation_codes"]=="1"){
					...
					if($required != "" || $this->pie_post_array['invitation'] != "")
					{
						...
						if(!empty($this->pie_post_array['invitation']) && count($codes) != 1)
						{
							$validation_message	= (!empty($field['validation_message']) ? $field['validation_message'] : __('Invalid invitation code','pie-register' ));
							$errors->add( $slug , apply_filters("piereg_invalid_invitaion_code",'<strong>'.esc_html(ucwords(__('error','pie-register'))).'</strong>: '.$validation_message));
				...
				if( !empty($privatekey) && intval($settings['piereg_enable_recaptcha']) ){
					...
					if($resp['success'] == false){
						$errors->add('recaptcha_mismatch',"<strong>".esc_html(ucwords(__('error','pie-register')))."</strong>: ". apply_filters("piereg_Invalid_Security_Code",__("Invalid Security Code", 'pie-register')));
					}
				}
			...
			{ 
				if(isset($_POST['piereg_math_captcha']))
				{
					...
					if( ($cookie_result1 == $cookie_result2) && ($cookie_result3 == $_POST['piereg_math_captcha'])){
						...
						$errors->add('math_captcha_mismatch',"<strong>".esc_html(ucwords(__('error','pie-register')))."</strong>: ". apply_filters("piereg_Invalid_Security_Code",__("Invalid Math Captcha", 'pie-register')));
					}
				}
			...
			{
				if( $field['type'] == "honeypot" ) continue; //Honeypot is only available in premium version
				...
				if( $field['type']=="name" && ( (!isset($field_name_lst) || empty($field_name_lst)) && $required) )
				{
					...
					$errors->add( $slug."_lst" , "<strong>". esc_html(ucwords(__("error","pie-register"))).":</strong> " .$validation_message_lst );
				}
	...
	function addUser($user_id, $form_id = 0)
	{
		...
		$form_user_verification             = $this->piereg_get_verification_type($this->data);
		...
		if(is_array($this->data)){
			...
			foreach($this->data as $field)
			{
				...
				if(!isset($field['meta']))
				{
					...
					{
						if ( $form_user_verification == "0" || $form_user_verification == "4" )
						{
							$prefix		= $wpdb->prefix."pieregister_";
							$codetable	= $prefix."code";				
							$codes 		= $wpdb->query( $wpdb->prepare("update $codetable set count = count + 1 where BINARY name = %s and status = %d", htmlentities($this->pie_post_array['invitation']), 1) );
							if(isset($wpdb->last_error) && !empty($wpdb->last_error)){
								$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
							}	
						}
	...
	function piereg_get_verification_type($form_data){
		$option 	                = get_option(OPTION_PIE_REGISTER);
		$option_user_verification 	= $option['verification'];
		$form_user_verification 	= isset($form_data['form']['user_verification']) ? $form_data['form']['user_verification'] : "";
		...
		return $option_user_verification;
	}
