...
$newpasspageLock = 0;
        	...
        	{
            if ( 'loggedout' == $action )
                $login_warning = '<strong>'.esc_html(ucwords(__("Warning","pie-register"))).'</strong>: '.apply_filters("piereg_now_logout",__("You are now logged out.","pie-register"));

            elseif ( 'recovered' == $action )
                $login_success = '<strong>'.esc_html(ucwords(__("success","pie-register"))).'</strong>: '.apply_filters("piereg_check_yor_emailconfrm_link",__("Check your e-mail for the confirmation link.","pie-register"));

			elseif ( 'payment_cancel' == $action )
                $login_warning = '<strong>'.esc_html(ucwords(__("Warning","pie-register"))).'</strong>: '.apply_filters("piereg_canelled_your_registration",__("You have cancelled the registration.","pie-register"));

			elseif ( 'payment_success' == $action )
                $login_success = '<strong>'.esc_html(ucwords(__("success","pie-register"))).'</strong>: '.apply_filters("piereg_thank_you_for_registration",__("Thank you for registering. Login credentials will be sent soon.","pie-register"));		
			...
			elseif ( 'activate' == $action )
			{
				...
				if(sizeof($unverified )==1)
				{
					...
					{
						$obj_PieRegister = new PieRegister();

						if ( !$obj_PieRegister->InvCodeExpired($user_id) ) 
						{
							...
							{
								$login_success = '<strong>'.esc_html(ucwords(__("success","pie-register"))).'</strong>: '.apply_filters("piereg_your_account_is_now_active",__("Your account is now active. Please check your email for login credentials.","pie-register"));
	
							}else{
								$login_success = '<strong>'.esc_html(ucwords(__("success","pie-register"))).'</strong>: '.apply_filters("piereg_your_account_is_now_active",__("Your account is now active","pie-register"));
							}
							...
							$login_error = '<strong>'.esc_html(ucwords(__("error","pie-register"))).'</strong>: '.apply_filters("piereg_invitation_code_expired",__("The account cannot be activated. Please contact site administrator","pie-register"));
						}
					}
					else
					{
						$login_error = '<strong>'.esc_html(ucwords(__("error","pie-register"))).'</strong>: '.apply_filters("piereg_invalid_activation_key",__("Invalid activation key","pie-register"));
					...
					if($user){
						...
						if(isset($user_meta[0]) && $user_meta[0] == 1){
							$login_warning = '<strong>'.esc_html(ucwords(__("Warning","pie-register"))).'</strong>: '.apply_filters("piereg_canelled_your_registration",__("This account is already active.","pie-register"));
							unset($user_meta);
							unset($user_name);
							unset($user);
						}
			...
			elseif ( 'resetpass' == $action || 'rp' == $action ){
				...
				if ( is_wp_error($user) ) {
					if ( $user->get_error_code() === 'expired_key' )
						$login_error = '<strong>'.esc_html(ucwords(__("error","pie-register"))).'</strong>: '.apply_filters("piereg_you_key_has_been_expired",__("This link has expired, please reset the password again.","pie-register").' <a href="'.esc_url(pie_lostpassword_url()).'" title="'.esc_attr__("Reset your password","pie-register").'">'.esc_html__("Lost your password?","pie-register").'</a>');
					else
						$login_error = '<strong>'.esc_html(ucwords(__("error","pie-register"))).'</strong>: '.apply_filters("piereg_this_reset_key_invalid_or_no_longer_exists",__("Reset key is invalid or has expired.","pie-register").' <a href="'.esc_url(pie_lostpassword_url()).'" title="'.esc_attr__("Reset your password","pie-register").'">'.esc_html__("Lost your password?","pie-register").'</a>');
						$newpasspageLock = 1;
				}else{
					$login_warning = '<strong>'.esc_html(ucwords(__("Warning","pie-register"))).'</strong>: '.__('Enter the new password below.',"pie-register");
				}
				...
				if( isset($_POST['pass1'], $_POST['piereg_get_password_nonce']) && wp_verify_nonce( $_POST['piereg_get_password_nonce'], 'piereg_wp_get_password_nonce' ) && !is_wp_error($user)){
					...
					if(isset($_POST['pass1']) && trim($_POST['pass1']) == ""){
						$login_error =  '<strong>'.esc_html(ucwords(__("error","pie-register"))).'</strong>: '.apply_filters("piereg_invalid_password",__( 'Invalid Password',"pie-register" ));
						$errors->add( 'password_reset_mismatch',$login_error );
					}elseif ( isset($_POST['pass1']) and strlen($_POST['pass1']) < 7  ){
						$login_error =  '<strong>'.esc_html(ucwords(__("error","pie-register"))).'</strong>: '.apply_filters("piereg_minimum_8_characters_required_in_password",__( 'The password must be at least 8 characters long',"pie-register" ));
						$errors->add( 'password_reset_mismatch',$login_error );
					}elseif ( isset($_POST['pass1']) && $_POST['pass1'] != $_POST['pass2'] ){
						$login_error =  '<strong>'.esc_html(ucwords(__("error","pie-register"))).'</strong>: '.apply_filters("piereg_the_passwords_do_not_match",__( 'The passwords do not match',"pie-register"));
						$errors->add( 'password_reset_mismatch',$login_error );
					}
					...
					if ( ( ! $errors->get_error_code() ) && isset( $_POST['pass1'] ) && !empty( $_POST['pass1'] ) ) {
						...
						$login_warning = '';
						$login_error = '';
						$login_success = '<strong>'.esc_html(ucwords(__("success","pie-register"))).'</strong>: '.apply_filters("piereg_your_password_has_been_reset",__( 'The password has been reset.' , "pie-register"));
					}
				}
			}
        }
		...
		if ( !empty($login_error) )
			$form_data .= '<p class="piereg_login_error"> ' . apply_filters('piereg_messages', wp_kses($login_error, $pie_register_base->piereg_forms_get_allowed_tags())) . "</p>";

		if ( !empty($login_success) )
			$form_data .= '<p class="piereg_message">' . apply_filters('piereg_messages',wp_kses($login_success, $pie_register_base->piereg_forms_get_allowed_tags())) . "</p>";
...
if ( isset($_GET['action']) && ('rp' == $action || 'resetpass' == $action) && ($newpasspageLock == 0) ){
	...
	$form_data .= '
	  <form name="resetpassform" class="piereg_resetpassform" action="'.esc_url(pie_modify_custom_url(pie_login_url(),'action=resetpass&key=' . $the_key . '&login=' . $the_user)).'" method="post" autocomplete="off">
		...
		<input type="hidden" id="user_login" value="'.esc_attr($this_user).'" autocomplete="off">';
...
if(!function_exists("pie_login_form_captcha"))
{
	function pie_login_form_captcha($value = 0,$piereg_widget = false){
		...
		if($value == 2){ // Math Captcha
			...
			$data = "";
			$data .='<div class="prMathCaptcha" data-cookiename="'. esc_attr($cookie).'" id="'.esc_attr($cap_id).'" style="display:inline-block;">';
			...
			if( ( $publickey && $recaptcha_type == 'v2') || ( $publickeyv3 && $recaptcha_type == 'v3' ) ){
				if($recaptcha_type == 'v2')
					$data .= '<div style="'.esc_attr($style_inline).'" class="piereg_recaptcha_widget_div" id="'.esc_attr($cap_id).'">';
				else
					$data .= '<div style="'.esc_attr($style_inline).'" class="piereg_recaptcha_widget_div">';

				if($recaptcha_type == 'v3')
					$data .= '<input type="hidden" name="g-recaptcha-response" id="'.esc_attr($cap_id).'" value="">';
				$data .= '</div>';
			}
			return $data;
