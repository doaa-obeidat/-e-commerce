...
function pieResetFormOutput($piereg_widget = false){
	...
	*/
	...
	$forgot_pass_form .= '<div class="'.esc_attr($classes).'">';
		...
		if (isset($pie_register_base->pie_post_array['reset_pass']) && trim($pie_register_base->pie_post_array['user_login']) != "")
		{
			...
			if(isset($option['captcha_in_forgot_value']) && $option['captcha_in_forgot_value'] == 1){
				if($option['capthca_in_forgot_pass'] == 2){
					...
					$invalidcaptchaerror = '<strong>'.esc_html(ucwords(__("error","pie-register"))).'</strong>: '. apply_filters('piereg_forgot_invalid_captcha_error',__('Invalid Captcha','pie-register')); # newlyAddedHookFilter
				...
				elseif($option['capthca_in_forgot_pass'] == 3){
					...
					$response = $pie_register_base->read_file_from_url("https://www.google.com/recaptcha/api/siteverify?secret=".trim($privatekey)."&response=".$captcha."&remoteip=".sanitize_text_field( wp_unslash( $_SERVER['REMOTE_ADDR'] ) ) );
				...
				if ($user_exists){
					...
					if($allow){
						...
						if ( (isset($option['user_enable_forgot_password_notification']) && $option['user_enable_forgot_password_notification'] == 1) && FALSE == wp_mail($user_email, $subject, $message,$headers)){
							$error[] =  '<strong>'.esc_html(ucwords(__("error","pie-register"))).'</strong>: '.__('The e-mail could not be sent. Please contact site administrator.','pie-register') ;
	...
	if ( (isset($error) && is_array($error) && count($error) == 0) && !empty($success) ) {
		...
		$forgot_pass_form .= wp_kses($success, $pie_register_base->piereg_forms_get_allowed_tags());
		$forgot_pass_form .= '</p></div>';
	} else if (isset($error) && is_array($error) && count($error) > 0 ) {
		$forgot_pass_form .= '<div class="alert"><p class="piereg_message '.esc_attr($error_class).'">';
		$forgot_pass_form .= wp_kses($error[0], $pie_register_base->piereg_forms_get_allowed_tags());
		$forgot_pass_form .= '</p></div>';
	} elseif($warning) {
		$forgot_pass_form .= '<div class="alert alert-warning"><p class="piereg_warning fp_desc">'.wp_kses($warning, $pie_register_base->piereg_forms_get_allowed_tags()).'</p></div>';
	}
		...
		if($value == 2){ // Math Captcha
			...
			if( ( $publickey && $recaptcha_type == 'v2') || ( $publickeyv3 && $recaptcha_type == 'v3' ) ){
				if($recaptcha_type == 'v2')
					$data .= '<div class="piereg_recaptcha_widget_div" id="'.esc_attr($cap_id).'" value="">';
				else
