...
function vw_kids_customize_register( $wp_customize ) {
    ...
    $wp_customize->add_control( new VW_Kids_Toggle_Switch_Custom_Control( $wp_customize, 'vw_kids_featured_image_hide_show', array(
		'label' => esc_html__( 'Featured Image','vw-kids' ),
		'section' => 'vw_kids_post_settings'
    )));
	...
	$wp_customize->add_setting( 'vw_kids_hide_show_footer_credit_link',array(
    	'default' => 1,
      	'transport' => 'refresh',
      	'sanitize_callback' => 'vw_kids_switch_sanitization'
    ));  
    $wp_customize->add_control( new VW_Kids_Toggle_Switch_Custom_Control( $wp_customize, 'vw_kids_hide_show_footer_credit_link',array(
      	'label' => esc_html__( 'Footer Credit Link','vw-kids' ),
      	'section' => 'vw_kids_footer'
    )));
	...
	*/
	public function sections( $manager ) {
		...
		$manager->add_section(new VW_Kids_Customize_Section_Pro($manager,'vw_kids_get_started_link',array(
			...
			'pro_url'  => esc_url('https://www.vwthemesdemo.com/docs/free-vw-kids/'),
		)));
	}
