jQuery(document).ready(function($) {
    ...
    $(document).on('click', '.vw-kids-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message vw-kids-plugin-install');
                button.addClass('button-primary vw-kids-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.vw-kids-recommended-plugins .vw-kids-activation-note').css('display','block');
    ...
    $(document).on('click', '.vw-kids-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .vw-kids-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .vw-kids-recommended-plugins').css('display','none');
    });
