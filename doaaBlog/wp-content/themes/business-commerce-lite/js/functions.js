/* global businessCommerceScreenReaderText */
/**
...
(function($) {
	...
	function initMainNavigation(container) {
		...
		var dropdownToggle = $('<button />', {
			...
			.append(businessCommerceScreenReaderText.icon)
			.append($('<span />', {
				'class': 'screen-reader-text',
				text: businessCommerceScreenReaderText.expand
			}));
		...
		container.find('.current-menu-ancestor > button')
			...
			.text(businessCommerceScreenReaderText.collapse);
		...
		container.find('.dropdown-toggle').on('click', function(e) {
			...
			// jscs:enable
			screenReaderSpan.text(screenReaderSpan.text() === businessCommerceScreenReaderText.expand ? businessCommerceScreenReaderText.collapse : businessCommerceScreenReaderText.expand);
		});
	}
	...
	(function() {
		...
		menuToggle.on('click.businessCommerce', function() {
	...
	(function() {
		...
		function toggleFocusClassTouchScreen() {
			if (window.innerWidth >= 910) {
				$(document.body).on('touchstart.businessCommerce', function(e) {
					if (!$(e.target).closest('.main-navigation li').length) {
						$('.main-navigation li').removeClass('focus');
					}
				});
				siteNavigation.find('.menu-item-has-children > a, .page_item_has_children > a').on('touchstart.businessCommerce', function(e) {
				...
				siteNavigation.find('.menu-item-has-children > a').off('touchstart.businessCommerce');
			}
		}

		if ('ontouchstart' in window) {
			$(window).on('resize.businessCommerce', toggleFocusClassTouchScreen);
			toggleFocusClassTouchScreen();
		}

		siteNavigation.find('a').on('focus.businessCommerce blur.businessCommerce', function() {
  ...
  //category Toggle
	...
	$(document).ready(function() {
		$(window).on('load.businessCommerce resize.businessCommerce', function() {
			if ( window.innerWidth < 910 ) {
