...
class Business_Commerce_Customizer_Utilities {
	/**
	...
	public static function register_option( $option ) {
		...
		$defaults = apply_filters( 'business_commerce_customizer_defaults', array() );
		...
		if ( ! empty( $get_posts ) ) {
			foreach ( $get_posts as $get_post ) {
				...
				if ( empty( $title ) ) {
					...
					$title = sprintf( esc_html__( 'ID: %s No Title', 'business-commerce-lite' ), $get_post->ID );
				}
	...
	static function section_visibility() {
		$options = array(
			'disabled'     => esc_html__( 'Disabled', 'business-commerce-lite' ),
			'homepage'     => esc_html__( 'Homepage / Frontpage', 'business-commerce-lite' ),
			'entire-site'  => esc_html__( 'Entire Site', 'business-commerce-lite' ),
		);

		return apply_filters( 'business_commerce_section_visibility_options', $options );
	}
}
 ...
 * Initialize class
 */
$business_commerce_utilities = new Business_Commerce_Customizer_Utilities();
