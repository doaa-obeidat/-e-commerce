 ...
 */
class Business_Commerce_Customizer {
	...
	public function register_options( $wp_customize ) {
		...
		$section_visibility = Business_Commerce_Customizer_Utilities::section_visibility();

		Business_Commerce_Customizer_Utilities::register_option(
			array(
				'settings'          => 'business_commerce_header_image_visibility',
				'type'              => 'select',
		...
		$wp_customize->add_section( new Business_Commerce_Upsell_Section( $wp_customize, 'upsell_section',
			array(
				'title'           => esc_html__( 'Business Commerce Available(Pro version)', 'business-commerce-lite' ),
				'url'             => 'https://fireflythemes.com/themes/business-commerce',
	...
	public function preview_js() {
		...
		wp_enqueue_script( 'business-commerce-customizer', get_template_directory_uri() . '/js/customizer-preview' . $min . '.js', array( 'customize-preview' ), business_commerce_get_file_mod_date( '/js/customizer-preview' . $min . '.js' ), true );
	}

	/**
	 * Binds the JS listener to make Customizer business_commerce_color_scheme control.
	 *
	 * @since 1.0
	 */
	public function customize_control_js() {
		...
		// Enqueue Select2.
		wp_enqueue_script( 'business-commerce-select2-js', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/select2' . $min . '.js', array( 'jquery' ), '4.0.13', true );
		wp_enqueue_style( 'business-commerce-select2-css', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'css/select2' . $min . '.css', array(), '4.0.13', 'all' );

		// Enqueue Custom JS and CSS.
		wp_enqueue_script( 'business-commerce-custom-controls-js', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/customizer' . $min . '.js', array( 'jquery', 'jquery-ui-core', 'business-commerce-select2-js' ), business_commerce_get_file_mod_date( '/js/customizer' . $min . '.js' ), true );
		
		wp_enqueue_style( 'business-commerce-custom-controls-css', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'css/customizer' . $min . '.css', null, business_commerce_get_file_mod_date( '/css/customizer' . $min . '.css' ), 'all' );
 ...
 */
$business_commerce_customizer = new Business_Commerce_Customizer();
